...
public record TopkaCommand(Top main) implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        final AtomicInteger topNum = new AtomicInteger();
        final List<Component> topNames = new ArrayList<>();

        entryMap(statManager().getBlocksBroken())
                .forEach(broken -> topNames.add(Component.text(topNum.incrementAndGet() + ". " + Bukkit.getOfflinePlayer(broken.getKey()).getName() + ": " + broken.getValue())));
        buildAndClear(blocksBroken, topNames, topNum);

        entryMap(statManager().getPlayersKilled())
                .forEach(killed -> topNames.add(Component.text(topNum.incrementAndGet() + ". " + Bukkit.getOfflinePlayer(killed.getKey()).getName() + ": " + killed.getValue())));
        buildAndClear(kills, topNames, topNum);

        entryMap(statManager().getDeaths())
                .forEach(deaths -> topNames.add(Component.text(topNum.incrementAndGet() + ". " + Bukkit.getOfflinePlayer(deaths.getKey()).getName() + ": " + deaths.getValue())));
        buildAndClear(death, topNames, topNum);

        if (statManager().getBlocksBroken().isEmpty()) blocksBroken.lore(Component.text("Brak statystyk.")).glow();
        if (statManager().getPlayersKilled().isEmpty()) kills.lore(Component.text("Brak statystyk.")).glow();
        if (statManager().getDeaths().isEmpty()) death.lore(Component.text("Brak statystyk.")).glow();
